...
class Synthesizer_Split:
	def __init__(self, hparams, gta=False, model_name='Tacotron'):
	...
	def synthesize(self, texts, basenames, out_dir, log_dir, mel_filenames, return_wav=False, wav_format='wav'):
		...
		if basenames is None:
			...
			wav_name = 'temp.wav'
			audio.save_wav(wav, wav_name, sr=hparams.sample_rate) #Find a better way
			if return_wav:
				return wav_name
		...
		for i, mel in enumerate(mels):
			...
			linear_filename = os.path.join(out_dir, 'linear-{}.npy'.format(basenames[i]))
			np.save(linear_filename, linears[i], allow_pickle=False)
	...
	def _get_output_lengths(self, stop_tokens):
		...
		rows = [np.round(stop_token).tolist() for stop_token in stop_tokens]
		output_lengths = [row.index(1) if 1 in row else len(row) for row in rows]
