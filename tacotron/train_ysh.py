...
from tacotron.feeder_ysh import Feeder
...
def model_train_mode(args, feeder, hparams, global_step, is_training=True):
    with tf.variable_scope('Tacotron_model', reuse=tf.AUTO_REUSE) as scope:
        ...
        if hparams.predict_linear:
            model.initialize(feeder.inputs, feeder.input_lengths, feeder.mel_targets, feeder.token_targets, linear_targets=feeder.linear_targets,
                ...
                is_training=is_training, split_infos=feeder.split_infos)
        else:
...
def train(log_dir, args, hparams):
    save_dir = os.path.join(log_dir, 'taco_pretrained', args.name)
    plot_dir = os.path.join(log_dir, 'train_plots', args.name)
    wav_dir = os.path.join(log_dir, 'train_wavs', args.name)
    mel_dir = os.path.join(log_dir, 'mel-spectrograms', args.name)
    eval_dir = os.path.join(log_dir, 'eval-dir', args.name)
    eval_plot_dir = os.path.join(log_dir, 'eval_plots', args.name)
    eval_wav_dir = os.path.join(log_dir, 'eval_wavs', args.name)
    tensorboard_dir = os.path.join(log_dir, 'tacotron_events', args.name)
    log(f'tensorboard save at{tensorboard_dir}')
    meta_folder = os.path.join(log_dir, 'metas', args.name)
    ...
    if hparams.predict_linear:
        linear_dir = os.path.join(log_dir, 'linear-spectrograms', args.name)
    ...
    log(f'use {hparams.tacotron_num_gpus} gpu process {len(feeder._metadata)} examples, one epoch will contains'
        f' {len(feeder._metadata) / 32 / hparams.tacotron_num_gpus} examples')
    #Set up model:
    global_step = tf.Variable(0, name='global_step', trainable=False)
    model, stats = model_train_mode(args, feeder, hparams, global_step, is_training=hparams.is_training)
    ...
    with tf.Session(config=config) as sess:
        try:
            ...
            while not coord.should_stop() and step < args.tacotron_train_steps:
                ...
                if step % args.summary_interval == 0 or first_step:
                    log('\nWriting summary at step {}'.format(step))
                    summary_writer.add_summary(sess.run(stats), step)

                if step % args.eval_interval == 0 or first_step:
