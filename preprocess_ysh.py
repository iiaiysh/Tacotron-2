...
def norm_data(args):
	...
	supported_datasets = ['LJSpeech-1.0', 'LJSpeech-1.1', 'M-AILABS', 'Blizzard2012']
	...
	if args.dataset.startswith('LJSpeech'):
		return [os.path.join(args.base_dir, args.dataset_path)]

	if args.dataset.startswith('Blizzard'):
		return [os.path.join(args.base_dir, args.dataset_path)]
...
def main():
	...
	parser.add_argument('--dataset', default='LJSpeech-1.1', choices=['Blizzard2012', 'LJSpeech-1.1'])
	parser.add_argument('--dataset_path', required=True)
	parser.add_argument('--output', default='training_data', required=True)
