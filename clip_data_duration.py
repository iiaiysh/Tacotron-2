...
for line in lines:
    ...
    ratio1 = duration/len_word
    res.append((wav_path ,duration, ratio, ratio1))
...
print('ratios of letter:')
print(max(ratios))
print(min(ratios))
print(sum(ratios)/len(ratios))
print('\n')

ratios1 = [item[3] for item in res]
print('ratios of word:')
print(max(ratios1))
print(min(ratios1))
print(sum(ratios1)/len(ratios1))
...
plt.title('ratio of letter')
plt.show()


n, bins, patches = plt.hist(x=ratios1, range=[0,1.5])
plt.title('ratio of word')
plt.show()