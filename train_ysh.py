...
from tacotron.train_ysh import tacotron_train
from wavenet_vocoder.train_ysh import wavenet_train
...
def prepare_run(args):
    ...
    log_dir = os.path.join(args.base_dir, f'logs-rayhane-{args.model}')
    os.makedirs(log_dir, exist_ok=True)
    modified_hp.add_hparam('time_str', time_string())
    os.makedirs(os.path.join(log_dir, f'Terminal_log/{args.name}'), exist_ok=True)

    infolog.init(os.path.join(log_dir, f'Terminal_log/{args.name}', f'Terminal_train_log_{modified_hp.time_str}'), args.name, args.slack_url)
...
def main():
    ...
    parser.add_argument('--tacotron_train_steps', type=int, default=5000000, help='total number of tacotron training steps')
